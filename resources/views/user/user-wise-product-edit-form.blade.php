...
@section('title', 'User Product Edit Form')
@section('content')
            ...
            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                <div class="card">
                    <div class="card-header">
                        <div class="card-title">User Product Permission Update</div>
                    </div>
                    {!! Form::open(array('route' =>['user-wise-product.update',$single_data->id],'method'=>'PUT','files'=>true)) !!}
                    <div class="card-body">
                      <div class="col-md-12">
                              ...
                              <div class="col-md-4">
                                <div class="field-wrapper">
                                  <div class="input-group">
                                    <select name="user_id" class="form-control select2" required="" autocomplete="off">
                                      <option value="">Select user</option>
                                      @foreach($alluser as $user)
                                      <option value="{{$user->id}}" {{($single_data->user_id == $user->id) ? 'selected' : ''}}>{{$user->name}}</option>
                                      @endforeach
                                    </select>
                                  </div>
                                    ...
                                    <tr id="row_{{$row_num}}">
                                        ...
                                        <td style="border: 1px solid #fff">
                                            <input type="text" step="any" class="form-control quantity_{{$row_num}}"
                                                name="addmore[{{$row_num}}][quantity]" value="{{$data->quantity}}" id="quantity" >
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
